...
import shutil
from text_importer.scripts.patching.canonical_patch_1_uzh import (write_jsonlines_file, 
...
def scale_coords(coords: list[int], curr_res: str | int, des_res: str | int) -> list[int]:
    return [int(c*int(des_res)/int(curr_res)) for c in coords]

def convert_issue_coords(issue: dict[str, Any], res: dict[str, int | list]) -> tuple[dict[str, Any], bool]:
...
def convert_page_coords(page: dict[str, Any], res: dict[str, int | list]) -> tuple[dict[str, Any], bool]:
...
def main():
    ...
    logger.info("Patching titles %s: rescaling %s property at issue and page level", RERO_1_TITLES, PROP_NAME)

    logger.info(f"Fetching the list of titles to patch")
    ...
    patched_rero_issues = (
        rero_issues
            .map_partitions(lambda i_list: [(i, i['id'].split('-')[0]) for i in i_list])
            .map_partitions(
                lambda i_list: [
    ...
    patched_rero_pages = (
        rero_pages
            .map_partitions(lambda p_list: [(p, p['id'].split('-')[0]) for p in p_list])
            .map_partitions(
                lambda p_list: [
                    find_convert_coords(p, np, all_to_patch[np], all_to_inv[np], is_issue=False) 
                    for (p, np) in p_list
                ]
            )
    ).persist()
    ...
    rero_issue_files = (
        patched_rero_issues
            .map_partitions(lambda i_l: [i[0] for i in i_l])
    ...
    rero_page_files = (
        patched_rero_pages
            .map_partitions(lambda pages: [p[0] for p in pages])
    ...
    logger.info("Aggregating the patching information of issues for future reference...")
    ...
    patched_info_merged_df.to_csv(os.path.join(final_patches_output_path, 'all_patched_issues.csv'))
