...
≈20 req/min) **inside the request lock**, so concurrent workers cannot burst past
the rate (a prior bug slept *outside* the lock, letting 6 workers fire near-
simultaneously and trip the limit). It retries with exponential backoff on request
exceptions and on transient statuses (`is_transient_status`: 429 and all 5xx —
including Cloudflare 52x such as **525**). On `429` it waits **≥60 s** (LOC blocks
for up to an hour) or honors `Retry-After`. `404` is deliberately *not* retried
since callers use it to probe batch/LCCN existence. `download_file` streams to a
`.part` temp file, renames on success, and retries mid-stream
`ChunkedEncodingError`/`ConnectionError`/`Timeout` drops.
